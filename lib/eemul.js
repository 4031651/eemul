...
eemul.keydown = function (selector, key) {
    ...
    var kdown  = document.createEvent("KeyboardEvent"),
        fn = typeof kdown.initKeyboardEvent == 'function' ? 'initKeyboardEvent' : 'initKeyEvent';

    kdown[fn]('keydown', true, true, window, false, false, false, false, key, eemul.undef);
...
eemul.keypress = function (selector, key) {
    ...
    var kpress = document.createEvent("KeyboardEvent"),
        fn = typeof kpress.initKeyboardEvent == 'function' ? 'initKeyboardEvent' : 'initKeyEvent';

    kpress[fn]('keypress', true, true, window, false, false, false, false, key, eemul.undef);
...
eemul.keyup = function (selector, key) {
    ...
    var kup = document.createEvent("KeyboardEvent"),
        fn = typeof kup.initKeyboardEvent == 'function' ? 'initKeyboardEvent' : 'initKeyEvent';
        

    kup[fn]('keyup', true, true, window, false, false, false, false, key, eemul.undef);
        ...
        th  = window.setInterval(function () {
            if (idx < len) {
                var kdown  = document.createEvent("KeyboardEvent"),
                    ...
                    _fn    = typeof kup.initKeyboardEvent == 'function' ? 'initKeyboardEvent' : 'initKeyEvent';

                kdown[_fn]('keydown', true, true, window, false, false, false, false, str.charCodeAt(idx), 0);
                elem.dispatchEvent(kdown);
                kpress[_fn]('keypress', true, true, window, false, false, false, false, str.charCodeAt(idx), 0);
                elem.dispatchEvent(kpress);
                $elem.val($elem.val() + str.charAt(idx));
                kup[_fn]('keyup', true, true, window, false, false, false, false, str.charCodeAt(idx), 0);
                elem.dispatchEvent(kup);
                idx++;
            } else {
                window.clearInterval(th);
