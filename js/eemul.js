 ...
 */
eemul.click = function (selector) {
    var elem = jQuery(selector).get(0);
    if (!elem) {
        return;
    }
    var evt = document.createEvent("MouseEvents"),
        offset = eemul.findCenter(elem);

    evt.initMouseEvent("click", true, true, window, 1, offset.left, offset.top, offset.left, offset.top, false, false, false, false, 0, null);
...
eemul.dragTo = function (selector, dX, dY, time, steps) {
    ...
    mdown.initMouseEvent("mousedown", true, true, window, 1, offset.left, offset.top, offset.left, offset.top, false, false, false, false, 0, null);
    elem.dispatchEvent(mdown);

    var mmove = null, mup = null, click = null,
        th = window.setInterval(function () {
        if (far(x, offset.left + dX, incX / 2) || far(y, offset.top + dY, incY / 2)) {
            ...
            mmove.initMouseEvent("mousemove", true, true, window, 1, offset.left + dX, offset.top + dY, offset.left + dX, offset.top + dY, false, false, false, false, 0, null);
            elem.dispatchEvent(mmove);
            mup.initMouseEvent("mouseup", true, true, window, 1, offset.left + dX, offset.top + dY, offset.left + dX, offset.top + dY, false, false, false, false, 0, null);
            elem.dispatchEvent(mup);
            click.initMouseEvent("click", true, true, window, 1, offset.left + dX, offset.top + dY, offset.left + dX, offset.top + dY, false, false, false, false, 0, null);
...
eemul.keydown = function (selector, key) {
    ...
    kdown.initKeyEvent('keydown', true, true, window, false, false, false, false, key, eemul.undef);
...
eemul.keypress = function (selector, key) {
    ...
    kpress.initKeyEvent('keypress', true, true, window, false, false, false, false, key, eemul.undef);
...
eemul.keyup = function (selector, key) {
    ...
    kup.initKeyEvent('keyup', true, true, window, false, false, false, false, key, eemul.undef);
 ...
 * @param {Number} time    The duration of the animation
 */
eemul.fill = function (selector, str, time) {
    var elem = jQuery(selector).get(0);
    if (!elem) {
        return;
    }
    ...
    var len = str.length,
        ...
        th  = window.setInterval(function () {
            if (idx < len) {
                ...
                kdown.initKeyEvent('keydown', true, true, window, false, false, false, false, str.charCodeAt(idx), 0);
                elem.dispatchEvent(kdown);
                kpress.initKeyEvent('keypress', true, true, window, false, false, false, false, str.charCodeAt(idx), 0);
                elem.dispatchEvent(kpress);
                $elem.val($elem.val() + str.charAt(idx));
                kup.initKeyEvent('keyup', true, true, window, false, false, false, false, str.charCodeAt(idx), 0);
                elem.dispatchEvent(kup);
                idx++;
            } else {
                window.clearInterval(th);
